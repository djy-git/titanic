...
cat_out_cols1 = ['Name', 'Ticket', 'Cabin', 'Embarked', 'First name', 'Last name']
cat_out_cols2 = ['A', 'F', 'T', 'G']
...
class NumFeatureAdder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        df_X = pd.DataFrame(X, columns=num_cols)  # ColumnTransformer() makes DataFrame to ndarray

        # Add 'log(Fare)' and 'Household'
        if self.log_Fare:
            df_X['log(Fare)'] = np.log(1 + df_X['Fare'])
        if self.Household:
...
class NumFeatureDropper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df_X, y=None):
        df_X.drop(columns=num_out_cols, inplace=True)
        return df_X
...
class CatFeatureAdder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        return self
    def transform(self, X, y=None):
        df_X = pd.DataFrame(X, columns=cat_cols)

        # Split 'Name' into ['Last name', 'Title', 'First name']
        names = self._split_name(df_X)
        df_X = df_X.join(names)

        # Combine 'Title' into ['Mr', 'Mrs', 'Miss']
        df_X['Title'] = df_X.apply(self._combine_title, axis=1)

        # Extract 'Deck' from 'Cabin'
        df_X['Cabin'] = df_X['Cabin'].fillna(value='Unknown')
        df_X = self._extract_deck(df_X)
        return df_X
    ...
    def _extract_deck(self, X):
        cabin_list = np.unique([cabin[0] for cabin in X['Cabin']])  # First letter
...
class CatFeatureDropper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if self.flag == 1:
            df_X = X
            df_X.drop(columns=cat_out_cols1, inplace=True)
        elif self.flag == 2:
            df_X = pd.DataFrame(X, columns=proc_cat_oh_cols)
            df_X.drop(columns=cat_out_cols2, inplace=True)
        return df_X
